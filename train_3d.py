...
def objective(trial):
    ...
    modes            = trial.suggest_int("modes", 2, 6)  # modes_x, modes_y, modes_z are the same
    width           = trial.suggest_int("width", 16, 32, step=8)  # width of the model
    ...
    for e in range(num_epochs):
        ...
        for i in range(0, train_x_set.shape[0], batch_size):
            if i + batch_size > inputs_train.shape[0]: # skip last batch to ensure sharding works
                break
        ...
        for i in range(0, inputs_validation.shape[0], batch_size):
            if i + batch_size > inputs_validation.shape[0]: # skip last batch to ensure sharding works 
                break
